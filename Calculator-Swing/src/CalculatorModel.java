...
public class CalculatorModel {
	...
	public String getResult() {
		...
		/**
		 * if the operational mode is F then it means there are three types of
		 * precisions which can be set Sci, .0 or, .00
		 */
		if (operationalMode.equals("F")) {
			...
			if (result.equalsIgnoreCase("Cannot divide by zero") || result.equalsIgnoreCase("Results not defined")) {
	...
	/**
	 * The following function calculates the result using operands, operator and
	 * operational mode.
	 *
	 * @return none
	 */
	...
	public void calculate() {
		...
		try {
			/*
			 * if the arithmetic operation is I i.e., Integer, then we get inside and switch
			 * the arithmetic operation and calculate and set the result
			 */
			...
			case "F": {
				/*
				 * if it is not Integer it means its a float, so here we are calculating the
				 * operands using float
				 */
